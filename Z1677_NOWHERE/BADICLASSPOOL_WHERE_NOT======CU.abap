...
*"* public components of class BADICLASSPOOL_WHERE_NOT
...
public section.
  ...
  interfaces IF_BADI_IFPOOL_WHERE_NOT .


  events BADI_EVENT_WHERE_NOT_KO2
    exporting
      value(I_MESSAGE) type STRING optional .

  events BADI_EVENT_WHERE_NOT_KO3
    exporting
      value(I_MESSAGE) type STRING optional .

  methods ON_BADI_EVENT_WHERE_NOT_KO2
    for event BADI_EVENT_WHERE_NOT_KO2 of BADICLASSPOOL_WHERE_NOT
    importing I_MESSAGE .

  methods ON_BADI_EVENT_WHERE_NOT_KO3
    for event BADI_EVENT_WHERE_NOT_KO3 of BADICLASSPOOL_WHERE_NOT
    importing I_MESSAGE .
    
  methods SEND_BADI_EVENT_WHERE_NOT_KO2
    importing I_MESSAGE type STRING optional .

  methods SEND_BADI_EVENT_WHERE_NOT_KO3
    importing I_MESSAGE type STRING optional .
